...
exports.delete = (req, res) => {
    ...
    Cliente.destroy({ where: { id: id } })
        .then(num => {
            if (num == 1) {
                console.log("se ha borrado el cliente con id ", id);
                res.send({
                    message: "El cliente fue borrado correctamente!"
                });
            } else {
                res.send({
                    message: `Ocurrió un error. No se pudo borrar el cliente con id= ${id}.!`
                });
            }
        })
        .catch(err => {
            res.status(500).send({
                message: "Error borrando el cliente con id= " + id
            });
        });
};